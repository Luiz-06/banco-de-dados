...
SELECT NUM_AP FROM HOSPEDAGEM 
WHERE COD_HOSP = (SELECT COD_HOSP FROM HOSPEDE WHERE NOME = 'JOAO') 
AND NUM_AP = (
    SELECT NUM_AP FROM APARTAMENTO 
    WHERE COD_CAT = (
        SELECT COD_CAT FROM CATEGORIA 
        WHERE PRECO = (SELECT MAX(PRECO) FROM CATEGORIA)
    )
);
