/*21. A relação com o nome dos hóspedes, a data de entrada, a data de 
saída e o valor total pago em díarias (não é necessário considerar a hora 
de entrada e saída, apenas a data*/
...
SELECT H.NOME, 
    HG.DT_ENT, 
    HG.DT_SAIDA, 
    SUM(C.PRECO * EXTRACT(DAY FROM (HG.DT_SAIDA - HG.DT_ENT))) AS VALOR_TOTAL
FROM HOSPEDE AS H
JOIN HOSPEDAGEM AS HG ON H.COD_HOSP = HG.COD_HOSP 
JOIN APARTAMENTO AS A ON HG.NUM_AP = A.NUM_AP
JOIN CATEGORIA AS C ON A.COD_CAT = C.COD_CAT
GROUP BY H.NOME, HG.DT_ENT, HG.DT_SAIDA;
